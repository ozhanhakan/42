...
{
    size_t i = 0;
    while (s && s[i]) // s NULL değilse ve karakter sonu gelmediyse
        i++;
    return (i);
}
...
char *ft_strchr(const char *s, int c)
{
    ...
    {
        if (*s == (char)c)
            return ((char *)s); // Karakter bulunduysa adresini döndür
        s++;
    }
    if (c == 0)
        return ((char *)s); // '\0' karakteri aranıyorsa string sonunu döndür
    return (NULL);
}
...
char *ft_strdup(const char *s)
{
    ...
    char *dup = malloc(len + 1); // +1 null karakter için
    if (!dup)
        return (NULL);
...
char *ft_strjoin_and_free(char *s1, char *s2)
{
    ...
    char *res = malloc(len1 + len2 + 1); // +1 null karakter için
    if (!res)
        return (NULL);
    ...
    for (size_t i = 0; i < len2; i++)
        res[len1 + i] = s2[i];
    res[len1 + len2] = '\0';
    free(s1); // s1'in belleği serbest bırakılır
    return (res);
}
